...
import { ListItemsContratoComponent } from './list-items-contrato/list-items-contrato.component';
...
@Component({
  selector: 'app-items-contrato',
  templateUrl: './items-contrato.component.html',
  styleUrls: ['./items-contrato.component.scss']
})
export class ItemsContratoComponent implements OnInit {
...
Vercot(): void {
  ...
  const dialogRef = this._modelService.open(ListItemsContratoComponent, {
    disableClose:true,
    autoFocus:true,
     width: '100%',
     height:'80%',
  ...
  dialogRef.afterClosed().subscribe(result => {
    ...
    if(id){
      ...
      const uintscant=this._items2.classification.unit.numreq;
      const uintscheme=this._items2.classification.unit.scheme;
      const unitname=this._items2.classification.unit.name;
      const uintvalor=this._items2.classification.unit.valor;
      const uintamount=this._items2.classification.unit.value.amount;
       const uintcurrency=this._items2.classification.unit.value.currency;
      const uinturi=this._items2.classification.unit.uri;
...
UptItem() {
  ...
  this._items.find(item => item.id ==form.id).classification.unit.numreq= form.uintscant;
  this._items.find(item => item.id ==form.id).classification.unit.scheme= form.uintscheme;
  this._items.find(item => item.id ==form.id).classification.unit.name= form.unitname;
  this._items.find(item => item.id ==form.id).classification.unit.valor= form.uintvalor;
  this._items.find(item => item.id ==form.id).classification.unit.value.amount= form.uintamount;
  this._items.find(item => item.id ==form.id).classification.unit.value.currency= form.uintcurrency;
  this._items.find(item => item.id ==form.id).classification.unit.uri= form.uinturi;
  this.botonUpdateItem = false;
  this.botonGuardaItem = true;
  this.botonVerItem= true;
